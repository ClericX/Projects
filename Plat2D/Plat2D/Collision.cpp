...
bool CCollision::Overlaps(int x0, int y0, int x1, int y1, int x2, int y2, int x3, int y3)
{
	return ( (! ( ((x0)<(x2) && (x1)<(x2)) || ((x0)>(x3) && (x1)>(x3)) || ((y0)<(y2) && (y1)<(y2)) || ((y0)>(y3) && (y1)>(y3)) ) ) );
}

// Object-to-object bounding-box collision detector
bool CCollision::RectToRect(Rect one, Rect two)
{
...
bool CCollision::PixelToPoint(CMapObject *MapObj, long X, long Y, D3DLOCKED_RECT* lpLockedRect)
{
...
bool CCollision::TextureToTexture(CMapObject *ObjOne, CMapObject *ObjTwo)
{
...
bool CCollision::PointToRect(POINT point, Rect rect)
{
